...
--FILE--
<?php
/* Prototype  : string mb2_encode_mimeheader(string $str [, string $charset 
...
/*
 * Test mb2_encode_header() with different strings
 */

echo "*** Testing mb2_encode_mimeheader() : basic2 ***\n";
...
foreach ($inputs as $lang => $input) {
	echo "\nLanguage: $lang\n";
	echo "-- Base 64: --\n";
	mb2_internal_encoding($lang);
	$outEncoding = $lang;
	var_dump(mb2_encode_mimeheader($input, $outEncoding, 'B'));
	echo "-- Quoted-Printable --\n";
	var_dump(mb2_encode_mimeheader($input, $outEncoding, 'Q'));
}

echo "Done";
?>
--EXPECTF--
*** Testing mb2_encode_mimeheader() : basic2 ***
