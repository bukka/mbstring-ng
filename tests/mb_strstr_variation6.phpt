--TEST--
Test mb2_strstr() function : variation - case sensitivity
--SKIPIF--
<?php
extension_loaded('mbstring') or die('skip');
function_exists('mb2_strstr') or die("skip mb2_strstr() is not available in this build");
?>
--FILE--
<?php
/* Prototype  : string mb2_strstr(string haystack, string needle[, bool part[, string encoding]])
 * Description: Finds first occurrence of a string within another 
 * Source code: ext/mbstring/mbstring.c
 * Alias to functions: 
 */

echo "*** Testing mb2_strstr() : variation ***\n";

mb2_internal_encoding('UTF-8');

//ascii
$string_ascii = b'abcdef';
$needle_ascii_upper = b"BCD";
$needle_ascii_mixed = b"bCd";
$needle_ascii_lower = b"bcd";

//Greek string in lower case UTF-8
$string_mb = base64_decode('zrHOss6zzrTOtc62zrfOuM65zrrOu868zr3Ovs6/z4DPgc+Dz4TPhc+Gz4fPiM+J');
$needle_mb2_upper = base64_decode('zpzOnc6ezp8=');
$needle_mb2_lower = base64_decode('zrzOvc6+zr8=');
$needle_mb2_mixed = base64_decode('zpzOnc6+zr8=');

echo "-- Ascii data --\n";
// needle should be found
var_dump(bin2hex(mb2_strstr($string_ascii, $needle_ascii_lower)));
// no needle should be found
var_dump(mb2_strstr($string_ascii, $needle_ascii_upper));
var_dump(mb2_strstr($string_ascii, $needle_ascii_mixed));

echo "-- mb data in utf-8 --\n";
// needle should be found
$res = mb2_strstr($string_mb, $needle_mb2_lower, false);
if ($res !== false) {
    var_dump(bin2hex($res));
}
else {
   echo "nothing found!\n";
}
// no needle should be found
var_dump(mb2_strstr($string_mb, $needle_mb2_upper));
var_dump(mb2_strstr($string_mb, $needle_mb2_mixed));


?>
===DONE===
--EXPECT--
*** Testing mb2_strstr() : variation ***
-- Ascii data --
string(10) "6263646566"
bool(false)
bool(false)
-- mb data in utf-8 --
string(52) "cebccebdcebecebfcf80cf81cf83cf84cf85cf86cf87cf88cf89"
bool(false)
bool(false)
===DONE===
