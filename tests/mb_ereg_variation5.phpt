--TEST--
Test mb2_ereg() function : usage variations - Test anchors in regex
--SKIPIF--
<?php
...
/*
 * Test mb2_ereg with anchors (start and end of string) in $pattern
 */
...
var_dump(mb2_ereg($regex, $string_ascii));
echo "ASCII String with \$regs arg:\n";
var_dump(mb2_ereg($regex, $string_ascii, $regs_ascii));
base64_encode_var_dump($regs_ascii);

echo "\nMultibyte String without \$regs arg:\t";
var_dump(mb2_ereg($regex, $string_mb));
echo "Multubyte String with \$regs arg:\n";
var_dump(mb2_ereg($regex, $string_mb, $regs_mb));
